...
def _str_to_bool(value: str | bool | None) -> bool | None:
    ...
    if normalized in {"false", "0", "no", "n"}:
        return False
    raise ValueError(f"Valor inválido para booleano: {value}")
...
def run_clientes_ai_end_to_end(
    *,
    ...
    config_path: str = "config/clientes_ai_pipeline.yml",
    ...
    base_logger = PlatformLogger(
        component="run_clientes_ai_end_to_end",
        env=ctx.env,
        project=ctx.project,
    )
    ...
    def _run(logger: PlatformLogger):
        logger.info("Executando ingestão de knowledge")
        run_ingest_knowledge(
            spark=spark,
            project=project,
            use_catalog=ctx.naming.use_catalog,
            config_path=config_path,
            parent_component="run_clientes_ai_end_to_end",
            parent_run_id=run_id,
            forced_run_id=str(uuid4()),
        )

        if cfg.get("chunking", {}).get("enabled", True):
            logger.info("Executando chunking de documentos")
            run_chunk_documents(
                spark=spark,
                project=project,
                use_catalog=ctx.naming.use_catalog,
        ...
        if cfg.get("embeddings", {}).get("enabled", True):
            logger.info("Executando geração de embeddings")
        ...
        if cfg.get("index", {}).get("enabled", True):
            logger.info("Executando construção de índice vetorial")
            run_build_index(
                spark=spark,
                project=project,
                use_catalog=ctx.naming.use_catalog,
        ...
        if cfg.get("evaluation", {}).get("enabled", True):
            logger.info("Executando avaliação RAG")
            run_evaluate_rag(
                spark=spark,
                project=project,
                use_catalog=ctx.naming.use_catalog,
        ...
        logger.info("Pipeline AI end-to-end concluído com sucesso")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Executa pipeline AI end-to-end do exemplo clientes."
    )
    parser.add_argument("--project", default="clientes")
    parser.add_argument(
        "--config-path",
        dest="config_path",
        default="config/clientes_ai_pipeline.yml",
    )
    parser.add_argument(
        "--use-catalog",
        dest="use_catalog",
